...
function dealingScreen(so, ctx) {
  ...
  const node = h('div', { class: 'stack so-screen' },
    ...
    dealMistake && h('div', { class: 'so-banner so-bad' },
      `${dealMistake.by === ctx.me.id ? 'You' : ctx.player(dealMistake.by).name} played ${dealMistake.card} — ${dealMistake.burned.join(', ')} burned`),
    table,
    handFan,
...
function overScreen(so, snap, ctx) {
  ...
  return h('div', { class: 'card win-screen so-over has-art' },
    sceneArt(so.won ? 'win-together' : 'win-alone', 'band'),
